...
- Unreal Engine = 4.25.4
    - Linux: [click here for installations instructions on linux from source code.](https://docs.unrealengine.com/en-US/SharingAndReleasing/Linux/BeginnerLinuxDeveloper/SettingUpAnUnrealWorkflow/index.html)
    - Note: Please do not build Unreal Engine from source code as root. Refusing to run with the root privileges errors will occur. Instead create another user and build Unreal Engine from that user. Change the ownership to yourself on all files using the following command:
    
    `sudo chown -R 'whoami' UnrealEngine/`
    
    -  To make sure that the owner has read/write permission on all files run:
    ...
    ![image](https://user-images.githubusercontent.com/47066093/111857819-28a87280-8902-11eb-830a-5a9178b63e53.png)


    - Now, run `GenerateProjectFiles.sh` from the terminal to generate your project files.
    
    ![image](https://user-images.githubusercontent.com/47066093/111857823-2fcf8080-8902-11eb-9cb8-b3631d6f4b76.png)


    - To build the project, run `make` from the terminal.

    ![image](https://user-images.githubusercontent.com/47066093/111857828-33fb9e00-8902-11eb-8064-82d397b24fa8.png)
...
Even tho the code is valid and compiles, VS Code may show red squiggles on #includes, to get rid of theses warnings copy de following at the `c_cpp_properties.json` file at the defines section:
...
# Code structure Overview
 ...
 ToonTanks/Source/ToonTanks directory contains all the classes made for this project.
 
 #### `CharacterBase.h` and `CharacterBase.cpp`
 
 `CharacterBase.h` and `CharacterBase.cpp` defines the CharacterBase class which is a derived class from Character.cpp Unreal Engine built in class ready to use MovementComponent functionality and physics. CharacterBase is the main player Tank class which will have a derive Blueprint class, it has various Components that allows mesh collisions, movement and more.
 
 
 #### `PawnBase.h`, `PawnBase.cpp`, `PawnTurret.h` and `PawnTurret.cpp`
 `PawnBase` and `PawnTurret` are child classes from `Pawn.cpp` class, Pawn class has basic movement functionality and is made of components also. `APawnTurret::CheckFireCondition()` checks if player is in range so it can fire. `APawnBase::Fire()` spawns a ProjectileBase actor and follows the target direction. `APawnBase::RotateTurret` handles the turret rotation of the actor.
 
 #### `ProjectileBase.h` and  `ProjectileBase.cpp`
Defines the ProjectileBase class which handles the bullet mesh and the particle trails in its components. `AProjectileBase::OnHit` handles the behavior of the projectile when hitting a target, applying damage, spawning particles and explode sound and also destroy.

#### `PlayerControllerBase.h` and `PlayerControllerBase.cpp`
`APlayerControllerBase::SetPlayerEnabledState` enables and disables the player's input.

#### `TankGameModeBase.h` and `TankGameModeBase.cpp`
This class handles the game start and end game conditions of the level. `ATankGameModeBase::ActorDied(AActor* DeadActor)` checks what type of actor died, if player died then go to lose condition.
...
- The project accepts user input and processes the input (this is done in the form of a keyboard interaction from the user, see `CharacterBase.cpp` from line 144).
